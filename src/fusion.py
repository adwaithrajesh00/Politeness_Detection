...
def load_inputs():
    ...
    # Optional: quick NaN/Inf guards (helps catch earlier pipeline issues)
...
# Building the 3 cocatinated feature set

def build_feature_sets(text, emoji, stick):
    # Phase 1: Text only
    X_text = text

    # Phase 2: Text + Emoji  -> concat columns
    X_text_emoji = np.concatenate([text, emoji], axis=1)

    # Phase 3: Text + Emoji + Sticker  -> concat columns
